...
class SubHandler(object):
    ...
    def event_notification(self, event):
        ...
        print(f"OPC UA CLIENT: Got new event:\n\tResult: {result}\n\tTime: {time} (GMT)\n\tExecution Result: {excecution_result}")
...
class HoloOpcUaClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            self.request_measurement_parent_node = await self.root.get_child(["0:Objects",
                                                                              f"{self.idx_ipm}:HolographyModule",
                                                                              f"{self.idx_swap}:Services"])

            self.request_measurement_node = await self.root.get_child(["0:Objects",
                                                                       f"{self.idx_ipm}:HolographyModule",
                                                                       f"{self.idx_swap}:Services",
                                                                       f"{self.idx_ipm}:RequestMeasurement"])

            # Get the event_object_node and the event_types:
            self.event_object_node = await self.root.get_child(["0:Objects",
                                                                f"{self.idx_ipm}:HolographyModule"])

            self.event_types = await self.client.nodes.base_event_type.get_child(f"{self.idx_swap}:ServiceFinishedEventType")
            ...
            self.handle = await self.sub.subscribe_data_change([self.request_measurement_node,
                                                                self.event_types,
...
async def loop(opcua_client, file_searcher):
    ...
    try:
        ...
        if file_path:
            ...
            json_data = json.dumps(json_data, indent=2)  # Convert the JSON data to a string
